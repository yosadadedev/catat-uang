...
export class DatabaseManager {
  ...
  private async insertDefaultCategories() {
    ...
    if ((result as any)?.count > 0) {
      ...
      await this.db.runAsync("UPDATE categories SET icon = 'bag' WHERE icon = 'shopping-bag'");
      return;
    }
  ...
  async updateTransaction(id: number, transaction: Partial<Transaction>) {
    ...
    const values = fields
      .map((field) => transaction[field as keyof Transaction])
  ...
  async updateCategory(id: number, category: Partial<Category>) {
    ...
    const values = fields
      .map((field) => category[field as keyof Category])
      .filter((value) => value !== undefined);
    const setClause = fields.map((field) => `${field} = ?`).join(', ');
